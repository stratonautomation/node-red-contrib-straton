module.exports = function (RED) {
    ...
    function StratonWSNode(config) {
        ...
        node.port_count = node.subs.length;
                        ...
                        if (pObj.cmd == "read2" && pObj.error == 200 && (typeof pObj.data !== 'undefined') && pObj.data.length > 0) {
                            ...
                            for (i = 0; i < pObj.data.length; i++) {
                                ...
                                for (j = 0; j < node.subs.length; j += 1) {
                                    var sub = node.subs[j];
                                    //if (pObj.data[i].name == getfullVarName(sub)) {
    ...
    RED.nodes.registerType("straton read", StratonWSNode);
    ...
    RED.httpAdmin.post('/stratonvarlist/:id/:remoteurl', RED.auth.needsPermission('stratonread.read'), function (req, res) {
